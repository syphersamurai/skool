...
import { GRADING_SYSTEM } from '@/lib/types';
...
export default function EditResultPage({ params }: { params: { id: string } }) {
    ...
    try {
      ...
      if (resultData) {
        setFormData({
          ...
          teacherRemarks: resultData.teacherComment,
          principalRemarks: resultData.principalComment,
  ...
  const handleSubmit = async (e: React.FormEvent, saveAsDraft: boolean = false) => {
    ...
    try {
      ...
      const averageScore = formData.subjects.length > 0 ? totalScore / formData.subjects.length : 0;
      ...
      const updatedResult = {
        ...
        teacherComment: formData.teacherRemarks,
        principalComment: formData.principalRemarks,
        status: saveAsDraft ? 'draft' : 'published',
      };
      
      await resultsService.update(params.id, updatedResult);
